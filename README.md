...
- RX: Captures raw Ethernet frames using `libpcap`. Allocates a fixed-size buffer from global **Packet Pool**, copies the data into it and pushes the pointer a worker's ring.
...
- TX: If packet should be forwarded, then the worker writes the raw packet back to the interface using `AF_PACKET` sockets.
